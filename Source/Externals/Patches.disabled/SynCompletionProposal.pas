...
//These are the reflections of the Options property of the CompletionProposal
    ...
    function GetAssignedList: TStrings;
    function GetPosition: Integer;
    procedure SetAssignedList(const Value: TStrings);
  ...
  public
    ...
    property DisplayType: SynCompletionType read FDisplayKind write SetDisplayKind;
    ...
    property AssignedList: TStrings read GetAssignedList write SetAssignedList;
...
//    BorderWidth := 2;
  ...
  begin
    NewLinesInWindow := (NewHeight - BorderHeight) div ItemHeight;
  ...
  NewHeight := ItemHeight * FLinesInWindow + BorderHeight;
end;
...
//  DoubleBuffered := True;
  ...
  FListBox.Font.Name := 'MS Sans Serif';
  FListBox.Font.Size := 8;
  FListBox.OnDrawItem := DoDrawItem;
  FListBox.OnClick := DoClickItem;
  FListBox.OnDblClick := DoDblClick;
  FListBox.OnKeyDown := DoKeyDown;
  FListBox.OnKeyPress := DoKeyPress;
...
const
  CS_DROPSHADOW = $20000;
begin
  if (DisplayType = ctCode) and (FResizeable) then
...
begin
  ...
  begin
    case Key of
      SYNEDIT_RETURN:
        if (FCompleteWithEnter) and Assigned(FOnValidate) then
          FOnValidate(Self, Shift, #0);

      SYNEDIT_TAB:
        if  (FCompleteWithTab) and Assigned(FOnValidate) then
          FOnValidate(Self, Shift, #0);
      ...
      SYNEDIT_RIGHT:
        begin
          if (CurrentEditor <> nil) then
          begin
            if CurrentEditor.CaretX <= Length(CurrentEditor.LineText) then
              C := CurrentEditor.LineText[CurrentEditor.CaretX]
            else
              C := #32;

            if IsWordBreakChar(C) then
            begin
              if Assigned(FOnCancel) then
                FOnCancel(Self)
            end else
...
begin
  if FMatchText and (Index >= 0) and (Index < AssignedList.Count) then
    Result := Integer(AssignedList.Objects[Index])
  else
    Result := Index;
end;
...
begin
  ...
  begin
    Result := -1;
    for i := 0 to AssignedList.Count - 1 do
      if Integer(AssignedList.Objects[i]) = Index then
      begin
        Result := i;
        break;
      end;
...
begin
  ...
  begin
    ...
    if (BorderStyle = bsSingle) then
      Dec(Height, 2);
    FLinesInWindow := Height div ItemHeight;
  end;
end;
...
{    if UseInsertList then
    ...
    begin
      if (FMatchText) and (not UseItemList) then
        CompareString := AssignedList[aIndex]
      else
  ...
  begin
    ...
    begin
      if MatchItem(i, True) then
        AssignedList.AddObject(FItemList[i], TObject(i));
    end;
  end;
...
begin
  ...
  begin
    ...
    if Visible and Assigned(FOnChangePosition) and (DisplayType = ctCode) then
      FOnChangePosition(Owner as TSynBaseCompletionProposal, LogicalToPhysicalIndex(Position));
...
procedure TSynBaseCompletionProposalForm.SetDisplayKind(const Value: SynCompletionType);
begin
  FDisplayKind := Value;
  if (FDisplayKind = ctCode) and (FResizeable) then
    FListBox.BorderStyle := bsNone
  else
...
begin
  ...
  AssignedList.Assign(Value);
  CurrentString := CurrentString;
end;

procedure TSynBaseCompletionProposalForm.SetMargin(const Value: Integer);
begin
  FMargin := Value;
  Margins.SetBounds(FMargin, FMargin, FMargin, FMargin);
  AlignWithMargins := (FMargin <> 0);
end;
...
procedure TSynBaseCompletionProposalForm.DoClickItem(Sender: TObject);
begin
  if (Assigned(FOnChangePosition)) and (DisplayType = ctCode) then
    FOnChangePosition(Owner as TSynBaseCompletionProposal, LogicalToPhysicalIndex(Position));
end;

procedure TSynBaseCompletionProposalForm.DoDblClick(Sender: TObject);
begin
  DblClick;
end;
...
//we need to do the same as the enter key;
  if (DisplayType = ctCode) and (Assigned(FOnValidate)) then
    FOnValidate(Self, [ssDouble], #0);                      //GBN 15/11/2001
end;

procedure TSynBaseCompletionProposalForm.DoDrawItem(Control: TWinControl; Index: Integer; Rect: TRect; State: TOwnerDrawState);
var
  Handled: boolean;
  SaveDCIndex: integer;
  s: string;
begin
  if (FDisplayKind = ctCode) then
    SaveDCIndex := SaveDC(FListBox.Canvas.Handle)
  else
    SaveDCIndex := 0;
  try
    ...
    begin
      ...
      begin
        if (FClSelect <> clDefault) then
          FListBox.Canvas.Brush.Color := FClSelect;
        if (FClSelectText <> clDefault) then
          FListBox.Font.Color := FClSelectText;
      end else
      begin
        if (fClBackGround <> clDefault) then
          FListBox.Canvas.Brush.Color := fClBackGround;
      end;
    ...
    if (Assigned(FOnPaintItem)) then
      FOnPaintItem(Self, LogicalToPhysicalIndex(Index), FListBox.Canvas, Rect, Handled);
    ...
    begin
      ...
      begin
        if (FFormattedText) then
          FormattedTextOut(FListBox.Canvas, Rect, s, (odSelected in State), FColumns, FImages)
        else
          TextRect(FListBox.Canvas, Rect, 0, 0, FListBox.Items[Index]);
      end else
      begin
        if (FDisplayKind = ctParams) then
          s := FormatParamList(s, CurrentIndex);

        FormattedTextOut(FListBox.Canvas, Rect, s, False, nil, FImages);
      end;
    end;
    ...
    if (FDisplayKind = ctCode) then
    begin
      RestoreDC(FListBox.Canvas.Handle, SaveDCIndex);
      FListBox.Canvas.Refresh;
    end;
  end;
end;
...
begin
  ...
  // Fire OnClick event
  TListBoxCracker(FListBox).Click;
end;
...
procedure TSynBaseCompletionProposalForm.WMNCHitTest(var Msg: TWMNCHitTest);
begin
  // Note: Message is never received when Aero is active
    inherited;
  if (Msg.Result = HTTOP) or (Msg.Result = HTLEFT) or (Msg.Result = HTTOPLEFT) or (Msg.Result = HTTOPRIGHT) or (Msg.Result = HTBOTTOMLEFT) then
    Msg.Result := HTCLIENT;

//  if (Msg.XPos < 7) or (Msg.YPos < 7) then
end;
...
begin
  ...
  begin
    ...
    OldShowCaret := CurrentEditor.AlwaysShowCaret;
    CurrentEditor.AlwaysShowCaret := CurrentEditor.Focused;
//      UpdateCaret;
  end;
  //GBN 28/08/2002
...
procedure TSynBaseCompletionProposalForm.DoKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  KeyDown(Key, Shift);
  Key := 0;
end;

procedure TSynBaseCompletionProposalForm.DoKeyPress(Sender: TObject; var Key: Char);
begin
  KeyPress(Key);
  Key := #0;
end;
...
procedure TSynBaseCompletionProposalForm.SetAssignedList(const Value: TStrings);
begin
...
function TSynBaseCompletionProposalForm.GetAssignedList: TStrings;
begin
  Result := FListBox.Items;
end;

function TSynBaseCompletionProposalForm.GetFont: TFont;
begin
  Result := FListBox.Font;
end;

function TSynBaseCompletionProposalForm.GetItemHeight: Integer;
begin
  Result := FListBox.ItemHeight;
end;

function TSynBaseCompletionProposalForm.GetPosition: Integer;
begin
...
*)
        ...
        tmpWidth := FWidth;
        tmpHeight := Form.FHeightBuffer + Form.ItemHeight * FNbLinesInWindow + BorderHeight;
      end;
    ctHint:
      begin
        BorderWidth := 2;
        tmpHeight := Form.ItemHeight * ItemList.Count + BorderWidth
          + 2 * Form.Margin;
    ...
    ctParams:
      begin
        BorderWidth := 2;
        tmpHeight := (Form.ItemHeight * ItemList.Count) + BorderWidth + 2 * Form.Margin + 2;
...
begin
  ...
  begin
    ...
    if ((F.CurrentEditor as TCustomSynEdit).Owner is TWinControl) and (TWinControl(TCustomSynEdit(F.CurrentEditor).Owner).Visible) then
    begin
      TWinControl(TCustomSynEdit(F.CurrentEditor).Owner).SetFocus;
    end;

    // TCustomSynEdit.SetFocus has been overridden and does not do what we expect; Causes cannot focus a disabled or...etc.
    if (TCustomSynEdit(F.CurrentEditor).HandleAllocated) then
      SetFocus(TCustomSynEdit(F.CurrentEditor).Handle);
    ...
    begin
      ...
      try
        ...
        begin
          ...
          begin
            // Old: (causes sporadic List index out of bounds)
            // if (Form.AssignedList.Count > Position) then
            // New:
            if (Form.AssignedList.Count > 0) and (Form.AssignedList.Count > Position) then
              //GBN 15/01/2002 - Added check to make sure item is only used when no EndChar
              if (InsertList.Count > Integer(Form.AssignedList.Objects[position])) and
                 ((scoEndCharCompletion in fOptions) or (EndToken = #0)) then
                Value := InsertList[Integer(Form.AssignedList.Objects[position])]
              else
                Value := SelText
            else
              Value := SelText;
        ...
        begin
          //GBN 15/01/2002 - Added check to make sure item is only used when no EndChar
          if (Form.AssignedList.Count > Position) and
             ((scoEndCharCompletion in FOptions) or (EndToken = #0)) then
            Value := Form.AssignedList[Position]
          else
            Value := SelText;
        end;
...
begin
  HandleOnValidate(Sender, [ssDouble], #0);
end;
