...
{$ENDIF}
    ...
    procedure SetIndentWidth(Value: Integer);
...
{$ENDIF}
  ...
  public
    ...
    property IndentWidth: integer read fIndentWidth write SetIndentWidth default 2;
...
procedure TCustomSynEdit.SetIndentWidth(Value: Integer);
begin
  Value := MinMax(Value, 1, 256);
  if (Value <> fIndentWidth) then
    fIndentWidth := Value;
end;
...
var
  ...
  begin
    ...
    while (Run[0] = #32) and (Result < FIndentWidth) do
...
//    if (Run[0] = #9) and (Result < FTabWidth) then
    if (Run[0] = #9) and (Result < FIndentWidth) then
      Inc(Result);
  end;
...
//    StrToDeleteLen := (FTabWidth + 2) * (e - BB.Line) + FTabWidth + 1;
    StrToDeleteLen := (FIndentWidth + 2) * (e - BB.Line) + FIndentWidth + 1;
    //                chars per line * lines-1    + last line + null char
...
begin
  ...
  begin
    ...
    Len := Length(Line);
    if Len = 0 then Exit;
    // SIGMA-3569: Fix for "Range check error" invoking Search at end of line.
    // Old:
    // if (XY.Char >= 1) and (XY.Char <= Len + 1) and IsIdentChar(Line[XY.Char]) then
    // New
    if (XY.Char >= 1) and (XY.Char <= Len) and IsIdentChar(Line[XY.Char]) then
    begin
      Stop := XY.Char;
